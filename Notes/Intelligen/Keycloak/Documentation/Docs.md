...
A good practice is to include the JavaScript adapter in your application using a package manager like NPM or Yarn. TheΞ’Β keycloak-jsΞ’Β package is available on the following locations:

- NPM:Ξ’Β  [https://www.npmjs.com/package/keycloak-js](https://www.npmjs.com/package/keycloak-js)
- Yarn:Ξ’Β  [https://yarnpkg.com/package/keycloak-js](https://yarnpkg.com/package/keycloak-js)

Alternatively, the library can be retrieved directly from the Keycloak server atΞ’Β /js/keycloak.jsΞ’Β and is also distributed as a ZIP archive.  
...
To use the JavaScript adapter you must first create a client for your application in the Keycloak Admin Console. Make sureΞ’Β publicΞ’Β is selected forΞ’Β Access Type. You achieve this in Capability config by turning OFF client authentication toggle.  
==You also need to configure==Ξ’Β ==Valid Redirect URIs==Ξ’Β ==and==Ξ’Β ==Web Origins. Be as specific as possible as failing to do so may result in a security vulnerability.==  
Once the client is created click on theΞ’Β ActionΞ’Β tab in the upper right corner and selectΞ’Β Download adapter config. SelectΞ’Β Keycloak OIDC JSONΞ’Β forΞ’Β Format OptionΞ’Β then clickΞ’Β Download. The downloadedΞ’Β keycloak.jsonΞ’Β file should be hosted on your web server at the same location as your HTML pages.  
...
[OpenID Connect](https://openid.net/connect/)Ξ’Β (OIDC) is an authentication protocol that is an extension ofΞ’Β [OAuth 2.0](https://datatracker.ietf.org/doc/html/rfc6749). While OAuth 2.0 is only a framework for building authorization protocols and is mainly incomplete, OIDC is a full-fledged authentication and authorization protocol. OIDC also makes heavy use of theΞ’Β [Json Web Token](https://jwt.io/)Ξ’Β (JWT) set of standards. These standards define an identity token JSON format and ways to digitally sign and encrypt that data in a compact and web-friendly way.  
There are really two types of use cases when using OIDC. The first is an application that asks the Keycloak server to authenticate a user for them. After a successful login, the application will receive anΞ’Β _identity token_Ξ’Β and anΞ’Β _access token_. TheΞ’Β _identity token_Ξ’Β contains information about the user such as username, email, and other profile information. TheΞ’Β _access token_Ξ’Β is digitally signed by the realm and contains access information (like user role mappings) that the application can use to determine what resources the user is allowed to access on the application.  
==The second type of use cases is that of a client that wants to gain access to remote services. In this case, the client asks Keycloak to obtain an==Ξ’Β _access token_Ξ’Β ==it can use to invoke on other remote services on behalf of the user. Keycloak authenticates the user then asks the user for consent to grant access to the client requesting it. The client then receives the==Ξ’Β _access token_==. This==Ξ’Β _access token_Ξ’Β ==is digitally signed by the realm. The client can make REST invocations on remote services using this==Ξ’Β _access token_==. The REST service extracts the==Ξ’Β _access token_==, verifies the signature of the token, then decides based on access information within the token whether or not to process the request.==
...
**Logging in**  
The end user interacts with a relying party (such as a website) that provides an option to specify an OpenID for the purposes of authentication; an end user typically has previously registered an OpenID (e.g.Ξ’Β alice.openid.example.org) with an OpenID provider (e.g.==Ξ’Β openid.example.org).1  
The relying party typically transforms the OpenID into a canonical URL form (e.g.Ξ’Β http://alice.openid.example.org/).

- With OpenID 1.0, the relying party then requests the HTML resource identified by the URL and reads an HTML link tag to discover the OpenID provider's URL (e.g.Ξ’Β http://openid.example.org/openid-auth.php). The relying party also discovers whether to use aΞ’Β delegated identityΞ’Β (see below).
- With OpenID 2.0, the relying party discovers the OpenID provider URL by requesting theΞ’Β XRDSΞ’Β documentΞ’Β (also called theΞ’Β YadisΞ’Β document) with the content typeΞ’Β application/xrds+xml; this document may be available at the target URL and is always available for a target XRI.==
...
TheΞ’Β checkid_immediateΞ’Β mode can fall back to theΞ’Β checkid_setupΞ’Β mode if the operation cannot be automated.  
First, the relying party and the OpenID provider (optionally) establish aΞ’Β shared secret, referenced by an==Ξ’Β associate handle, which the relying party then stores. If using theΞ’Β checkid_setupΞ’Β mode, the relying party redirects the end user's user-agent to the OpenID provider so the end user can authenticate directly with the OpenID provider.  
...
If the end user accepts the OpenID provider's request to trust the relying party, then the user-agent is redirected back to the relying party along with the end user's credentials. That relying party must then confirm that the credentials really came from the OpenID provider. If the relying party and OpenID provider had previously established a shared secret, then the relying party can validate the identity of the OpenID provider by comparing its copy of the shared secret against the one received along with the end user's credentials; such a relying party is calledΞ’Β statefulΞ’Β because it stores the shared secret between sessions. In contrast, aΞ’Β statelessΞ’Β orΞ’Β dumbΞ’Β relying party must make one more background request (check_authentication) to ensure that the data indeed came from the OpenID provider.  
After the OpenID has been verified, authentication is considered successful and the end user is considered logged into the relying party under the identity specified by the given OpenID (e.g.Ξ’Β alice.openid.example.org). The relying party typically then stores the end user's OpenID along with the end user's other session information.==  
**Identifiers**  
To obtain an OpenID-enabledΞ’Β URLΞ’Β that can be used to log into OpenID-enabled websites, a user registers an OpenID identifier with an identity provider. Identity providers offer the ability to register a URL (typically a third-level domain, e.g. username.example.com) that will automatically be configured with OpenID authentication service.  
Once they have registered an OpenID, a user can also use an existing URL under their own control (such as a blog or home page) as an alias or "delegated identity". They simply insert the appropriate OpenID tags in theΞ’Β HTML13==Ξ’Β or serve aΞ’Β YadisΞ’Β document.14==  
...
XRIsΞ’Β are a new form ofΞ’Β InternetΞ’Β identifierΞ’Β designed specifically for cross-domain digital identity. For example, XRIs come in two formsΞ²β‚¬β€i-namesΞ’Β andΞ’Β i-numbersΞ²β‚¬β€that are usually registered simultaneously asΞ’Β synonyms. I-names are reassignable (like domain names), while i-numbers are never reassigned. When an XRI i-name is used as an OpenID identifier, it is immediately resolved to the synonymous i-number (the CanonicalID element of the XRDS document). This i-number is the OpenID identifier stored by the relying party. In this way, both the user and the relying party are protected from the end user's OpenID identity ever being taken over by another party as can happen with a URL based on a reassignable DNS name.

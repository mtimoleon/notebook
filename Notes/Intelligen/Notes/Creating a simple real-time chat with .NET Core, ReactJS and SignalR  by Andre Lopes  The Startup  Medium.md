...
Here Iβ€™m going to walk you through the creation of a simple real-time chat by having a .NET Core API and a ReactJS application connected and synchronized with SignalR.

# Requirements

- [.NET Core 3.1.5](https://dotnet.microsoft.com/download/dotnet-core/3.1)
- [NodeJS](https://nodejs.org/en/) β€” For **npm**
- Your favorite IDE/Text Editor β€” Here Iβ€™ll be using [Visual Studio Code](https://code.visualstudio.com/)

# The Project

Our project will be divided into two projects, one server Web API with .NET Core and one front-end with ReactJS. So letβ€™s create a folder Chatty to hold our whole project.  
Weβ€™ll start by developing our API.

# API

Letβ€™s create our API by navigating to the Chatty folder and creating a new folder called API. Inside this folder run the following command to create a new solution:  
dotnet new sln -n Chatty  
Now letβ€™s create a new webapi project and add it to our solution with:  
dotnet new webapi -o Chatty.Api  
dotnet sln add ./Chatty.Api/Chatty.Api.csproj

## SignalR

SignalR already comes shipped in .NET Core 3.1, so thereβ€™s no need to add any extra package in our API to use it.  
...
Now you might have noticed the β€magicalβ€ string ReceiveMessage. This might pose some issues like runtime errors if for some reason we misspell the correct name. To improve this code, we can extend the Hub\<T\> class instead of the default Hub class.  
The type this class receive is an interface that defines our ReceiveMessage action with its parameters.  
So letβ€™s create a folder Clients inside the Hubs folder and add an interface IChatClient.cs:  
...
To do so, letβ€™s add the following line to our ConfigureServices method in Startup.cs:  
services.AddSignalR();  
...
If you chose to not remove **HTTPS** redirection you will need to target the **HTTPS** address because SignalR doesnβ€™t allow redirections. In our case, we will be using the default local address [https://localhost:5001](https://medium.com/swlh/creating-a-simple-real-time-chat-with-net-core-reactjs-and-signalr-6367dcadd2c6).  
...
Now letβ€™s start by creating a ChatInput.js to render our user and message inputs:  
After that, we create a Message.js to display our message block and a ChatWindow.js to render ao chat messages:  
Awesome! We have our base components. Letβ€™s connect them and enable a SignalR connection. Letβ€™s create a Chat.js component:  
...
Now, if you open two tabs with the clientβ€™s address and fill the inputs with some information, you should see the messages showing on both views.  
Hereβ€™s a demo on the chat working.
...
To use it, letβ€™s first create a new controller ChatController.cs:  
Here we inject the interface IHubContext\<THub, T\> where THub is our ChatHub and T is our client type IChatClient.  
_Note that you can inject_ _Hub\<ChatHub\>__, but then you wonβ€™t be able to access the methods from your_ _IChatClient__, like_ _ReceiveMessage__._  
...
Now if you run the API and Client, youβ€™ll be getting the same result as before.
...
SignalR is Microsoftβ€™s technology that makes it easy to implement real-time applications. And the greatest thing is that you donβ€™t need to install any third-party packages because it is already embedded in the .NET Core framework.  
...
If youβ€™d like to see how the finished project from this article, you can get it [here](https://github.com/alopes2/Medium-Chatty).
